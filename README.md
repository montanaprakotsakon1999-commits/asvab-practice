...
- **📅 PLAN (v6.4)** — a one-year, 30-minutes-a-day study plan built in: today's tasks, a 53-week map, roadmap, streak, and a **▶ Start** button on every drill-able task that opens it right here (timed, or with answers). Practice sittings that include AR, WK, PC and MK log their AFQT and 92A line-score (CL) estimate to a score log automatically. Progress stays in your browser; save a backup file to move it to another device
...
- `tools/` — dev-only test pass, never loaded by the site. Run `npm install && npx playwright install chromium` once, then `npm test` before every push:
  ...
  - `tools/plansmoke.mjs` — 📅 PLAN: Start buttons, ticks + streak, automatic score log (checked against the results page), backup/load, 390 px
...
- **v6.4** — 📅 PLAN mode: the study plan built into the site, automatic AFQT + CL score log
