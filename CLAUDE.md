...
2. **Run the full test pass before every push — non-negotiable; it has caught real bugs in every round:**
   ```bash
   npm test          # validate.js + smoke.mjs + smoke54.mjs + gsmoke.mjs + mocksmoke.mjs (~5 min)
   ```
...
**Game script (ASVAB Arcade)** — masthead pill `[data-mode-sw="exam"|"game"]`, persisted in `localStorage asvab_mode`. Worlds: Mix, 209 Words, Word Power, Math Attack, Science Lab, Power Up (EI), Garage (AS), Gear Works (MC), Word Match. PC and AO are not in the game. XP `asvab_g_xp`, best scores `asvab_g_best`, mute `asvab_g_mute`.
...
- **`tools/gsmoke.mjs`** — game mode: switch + persistence, every world to its end screen, all Word Match boards with zero mistakes, home → hub, back to exam mode, 390 px.
