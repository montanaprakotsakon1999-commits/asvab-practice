...
2. **Run the full test pass before every push — non-negotiable; it has caught real bugs in every round:**
   ```bash
   npm test          # validate.js + smoke.mjs + smoke54.mjs + gsmoke.mjs + mocksmoke.mjs + studysmoke.mjs + wordsmoke.mjs + thsmoke.mjs (~7 min)
   ```
...
- **`tools/thsmoke.mjs`** — 🇹🇭 game: pill order + persistence, Thai data coverage (every why, template, AO tip, lesson, study sheet tags, dictionary word), bilingual hub/nodes/prompts/buttons/feedback/end, Thai explanation after answers (AR/WK/PC/AO), WK Thai meaning behind the hint + th-TH voice, PC cards, guides (Thai lines, Thai sheet + English toggle), language switch keeps the question, English game has no Thai, shared XP, 390 px.
